...
def pdf2doi(target):
    ...
    if path.isdir(target):
        ...
        if not (
            target.endswith(config.get("separator"))
...
def __find_doi(file: io.IOBase) -> dict:
    ...
    for method in config.get("finders_methods"):
        if method == "document_infos":
            # First method: we look into the pdf metadata (in the current implementation this is done
            # via the getDocumentInfo() method of the library PyPdf) and see if any of them is a string which contains a
            # valid identifier inside it. We first look for the elements of the dictionary with keys '/doi' or /pdf2doi_identifier'(if they exist),
        ...
        if method == "first_N_characters_google":
...
def save_identifiers(filename_identifiers, results, clipboard=False):
    ...
    if isinstance(filename_identifiers, str):
        path_filename_identifiers = (
            path.dirname(results[0]["path"])
            ...
            for result in results:
                if result["validation_info"]:
                    text = text + "{:<15s} {:<40s} {:<10s}\n".format(
                        result["identifier_type"], result["identifier"], result["path"]
                    )
                else:
                    text = text + "{:<15s} {:<40s} {:<10s}\n".format(
                        "n.a.", "n.a.", result["path"]
...
def main():
    ...
    parser.add_argument(
        "path",
        help="Path (relative or absolute) of the target pdf file or of the targe folder.",
        metavar="path",
        nargs="*",
    )
    parser.add_argument(
